/*
Nessie API

Transactional Catalog for Data Lakes  * Git-inspired data version control * Cross-table transactions and visibility * Works with Apache Iceberg tables

API version: 0.104.3
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ContentKeyDetailsV2 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ContentKeyDetailsV2{}

// ContentKeyDetailsV2 struct for ContentKeyDetailsV2
type ContentKeyDetailsV2 struct {
	Key *ContentKeyV2 `json:"key,omitempty"`
	MergeBehavior *MergeBehaviorV2 `json:"mergeBehavior,omitempty"`
	Conflict *ConflictV2 `json:"conflict,omitempty"`
}

// NewContentKeyDetailsV2 instantiates a new ContentKeyDetailsV2 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewContentKeyDetailsV2() *ContentKeyDetailsV2 {
	this := ContentKeyDetailsV2{}
	return &this
}

// NewContentKeyDetailsV2WithDefaults instantiates a new ContentKeyDetailsV2 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewContentKeyDetailsV2WithDefaults() *ContentKeyDetailsV2 {
	this := ContentKeyDetailsV2{}
	return &this
}

// GetKey returns the Key field value if set, zero value otherwise.
func (o *ContentKeyDetailsV2) GetKey() ContentKeyV2 {
	if o == nil || IsNil(o.Key) {
		var ret ContentKeyV2
		return ret
	}
	return *o.Key
}

// GetKeyOk returns a tuple with the Key field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ContentKeyDetailsV2) GetKeyOk() (*ContentKeyV2, bool) {
	if o == nil || IsNil(o.Key) {
		return nil, false
	}
	return o.Key, true
}

// HasKey returns a boolean if a field has been set.
func (o *ContentKeyDetailsV2) HasKey() bool {
	if o != nil && !IsNil(o.Key) {
		return true
	}

	return false
}

// SetKey gets a reference to the given ContentKeyV2 and assigns it to the Key field.
func (o *ContentKeyDetailsV2) SetKey(v ContentKeyV2) {
	o.Key = &v
}

// GetMergeBehavior returns the MergeBehavior field value if set, zero value otherwise.
func (o *ContentKeyDetailsV2) GetMergeBehavior() MergeBehaviorV2 {
	if o == nil || IsNil(o.MergeBehavior) {
		var ret MergeBehaviorV2
		return ret
	}
	return *o.MergeBehavior
}

// GetMergeBehaviorOk returns a tuple with the MergeBehavior field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ContentKeyDetailsV2) GetMergeBehaviorOk() (*MergeBehaviorV2, bool) {
	if o == nil || IsNil(o.MergeBehavior) {
		return nil, false
	}
	return o.MergeBehavior, true
}

// HasMergeBehavior returns a boolean if a field has been set.
func (o *ContentKeyDetailsV2) HasMergeBehavior() bool {
	if o != nil && !IsNil(o.MergeBehavior) {
		return true
	}

	return false
}

// SetMergeBehavior gets a reference to the given MergeBehaviorV2 and assigns it to the MergeBehavior field.
func (o *ContentKeyDetailsV2) SetMergeBehavior(v MergeBehaviorV2) {
	o.MergeBehavior = &v
}

// GetConflict returns the Conflict field value if set, zero value otherwise.
func (o *ContentKeyDetailsV2) GetConflict() ConflictV2 {
	if o == nil || IsNil(o.Conflict) {
		var ret ConflictV2
		return ret
	}
	return *o.Conflict
}

// GetConflictOk returns a tuple with the Conflict field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ContentKeyDetailsV2) GetConflictOk() (*ConflictV2, bool) {
	if o == nil || IsNil(o.Conflict) {
		return nil, false
	}
	return o.Conflict, true
}

// HasConflict returns a boolean if a field has been set.
func (o *ContentKeyDetailsV2) HasConflict() bool {
	if o != nil && !IsNil(o.Conflict) {
		return true
	}

	return false
}

// SetConflict gets a reference to the given ConflictV2 and assigns it to the Conflict field.
func (o *ContentKeyDetailsV2) SetConflict(v ConflictV2) {
	o.Conflict = &v
}

func (o ContentKeyDetailsV2) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ContentKeyDetailsV2) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Key) {
		toSerialize["key"] = o.Key
	}
	if !IsNil(o.MergeBehavior) {
		toSerialize["mergeBehavior"] = o.MergeBehavior
	}
	if !IsNil(o.Conflict) {
		toSerialize["conflict"] = o.Conflict
	}
	return toSerialize, nil
}

type NullableContentKeyDetailsV2 struct {
	value *ContentKeyDetailsV2
	isSet bool
}

func (v NullableContentKeyDetailsV2) Get() *ContentKeyDetailsV2 {
	return v.value
}

func (v *NullableContentKeyDetailsV2) Set(val *ContentKeyDetailsV2) {
	v.value = val
	v.isSet = true
}

func (v NullableContentKeyDetailsV2) IsSet() bool {
	return v.isSet
}

func (v *NullableContentKeyDetailsV2) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableContentKeyDetailsV2(val *ContentKeyDetailsV2) *NullableContentKeyDetailsV2 {
	return &NullableContentKeyDetailsV2{value: val, isSet: true}
}

func (v NullableContentKeyDetailsV2) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableContentKeyDetailsV2) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


